...
class PluraDLBuilder(object):
    authorizedUsers = ['fraca7', 'phihag', 'rg3', 'FiloSottile', 'ytdl-org']

    def __init__(self, **kwargs):
        if self.repoName != 'plura-dl':
            raise BuildError('Invalid repository "%s"' % self.repoName)
        if self.user not in self.authorizedUsers:
            raise HTTPError('Unauthorized user "%s"' % self.user, 401)

        super(PluraDLBuilder, self).__init__(**kwargs)
    ...
    def build(self):
        ...
        super(PluraDLBuilder, self).build()
...
class Builder(PythonBuilder, GITBuilder, PluraDLBuilder, DownloadBuilder, CleanupTempDir, Null):
    pass
